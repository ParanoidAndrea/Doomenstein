...
//------------------------------------------------------------------------------------------------
void AddVertsForTextTriangles2D( std::vector<Vertex_PCU>& verts, const std::string& text, const Vec2& startMins, float cellHeight, const Rgba8& color, float cellAspect, bool isFlipped, float spacingFraction, int maxCharactersNum)
{
...
//	verts.reserve( verts.size() + estimatedNumberOfNewVerts );
    ...
    if (numGlyphs > maxCharactersNum)
	numGlyphs = maxCharactersNum;
	
	//Rgba8 blockColor = Interpolate(topColor, bottomColor, 0.5f);
